...
void cmd_sync(const std::vector<std::string> &subArgs) {
    ...
    ws.onConnect = [&]{
        ...
        if (treeId) {
            ...
            Negentropy<negentropy::storage::SubRange> ne(subStorage, frameSizeLimit);
            neMsg = ne.initiate();
        } else {
            Negentropy<negentropy::storage::Vector> ne(storageVector, frameSizeLimit);
            neMsg = ne.initiate();
        }
    ...
    ws.onMessage = [&](auto msgStr, uWS::OpCode opCode, size_t compressedSize){
        ...
        try {
            ...
            if (msg.at(0) == "NEG-MSG") {
                ...
                std::string okMsg;
                auto res = writePolicyPlugin.acceptEvent(cfg().relay__writePolicy__plugin, evJson, EventSourceType::Sync, url, Bytes32(), okMsg);
